...
int completed_request(package_header header, void* buf) {
    qelement *rv;
    unsigned int hash;
    int newfd, nbytes, status;
    void *package;
...
void open_request_forward(int socketfd, package_header header, void* buf, unsigned int hash) {
    qelement *rv;
    void *package;
    uint8_t transFinder;
    int newfd, status, nbytes;
    char ip4str[INET_ADDRSTRLEN + 1], port[8];

    if (hash <= (unsigned int) sucID || (sucID < nodeID && hash > (unsigned int) nodeID))
        newfd = connect_to(sucIP, sucPort);
    else {
        get_peer(ip4str, port, fingertable, hash);
        newfd = connect_to(ip4str, port);
    }
    if (newfd == -1) {
    ...
    // CASE 2b NACHRICHT VON CLIENT
        ...
        append_list(&transid_list, socketfd, transFinder, header.transid, hash);
...
void read_package(int socketfd, void* buf) {
    ...
    if (header.intern) {
        if (header.ack) {
            hash = completed_request(header, buf);
            if (hash >= 0) {
                update_fingertable(fingertable, (unsigned int) hash, header);
            }
            return;
        } else if (header.join) {
    ...
    if (hashmap.size == 0) {
        ...
        init_fingertable(fingertable, nodeID, sucID, sucIP, sucPort);
    }
    ...
    // CASE 2 PEER NICHT ZUSTÄNDIG
        open_request_forward(socketfd, header, buf, hash);
    }
    free(buf);
    return;
}
...
int main(const int argc, const char* argv[]) {
    ...
    if (argc <= 4) {
        ...
        preID = -1;
        sucID = -1;
        printf("send_chord_join\n");
        send_chord_join(argv[4], argv[5]);
    }
    printf("check_sockets\n");
    check_sockets();
    return 0;
}
