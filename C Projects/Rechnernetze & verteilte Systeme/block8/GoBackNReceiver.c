...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    if (lastReceivedSeqNo + 1 == data->seqNo && crcValid) {     // Wenn Packet korrekt & erwartetes Packet
      lastReceivedSeqNo = data->seqNo;                            // Update lastSeqNo
      sendAck(s, data, lastReceivedSeqNo + 1);                    // Sende Empfangsbestätigung

      writeBuffer(output, data);                                  // Schreibe empfangene Daten
      goodBytes += bytesRead - sizeof(*data);                     // Korrekt empfangene Daten updaten

      if (data->size == sizeof(*data)) {                          // Wenn letztes Paket...
        // weiteres ACK, falls erstes ACK verloren gegangen ist,
        //  damit Wahrscheinlichkeit steigt, dass sender terminiert
        sendAck(s, data, lastReceivedSeqNo + 1);
        fclose(output);                                             // schließe Buffer
        close(s);                                                   // schließe Socket
        printf("TotalBytes: %ld\nGoodBytes: %ld\n", totalBytes, goodBytes);
        freeGoBackNMessageStruct(data);                             // gebe Speicher frei
        break;                                                      // terminiere
      }
    } else {                                                    // sonst...
      sendAck(s, data, lastReceivedSeqNo + 1);                    // fordere Packet erwartetes Paket nochmal
    }
