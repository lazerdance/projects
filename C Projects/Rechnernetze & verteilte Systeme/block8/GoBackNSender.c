...
/* YOUR TASK: When are we finished? */
  while (lastAckSeqNo < veryLastSeqNo + 1) {
    ...
    if (FD_ISSET(s, &readfds)) {
      ...
      if (ack->seqNoExpected > lastAckSeqNo && crcValid) {
    ...
    if (FD_ISSET(s, &writefds)) {
      ...
      while (nextSendSeqNo - lastAckSeqNo < window && nextSendSeqNo <= getLastSeqNoOfBuffer(dataBuffer)) {
        ...
        timeradd(&timeout, &currentTime, &data->timeout);

        if (timercmp(&data->timeout, &timerExpiration, <)) {
