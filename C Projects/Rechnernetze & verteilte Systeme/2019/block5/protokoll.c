...
// marshall internal messages lookup/reply/join operations
unsigned int marshall_internal_request(void **result, package_header header){
        ...
        uint32_t myip_nb;
        uint8_t byte = 128 + header.join*16 + header.notify*8 + header.stabilize*4 + header.reply * 2 + header.lookup;
        memcpy(package, &byte, 1);                                                      // write first header byte
...
package_header unmarshall(void *buffer) {
    ...
    if (header.intern) {
        ...
        header.stabilize= (*byte >>2) & 0x1;
        header.notify   = (*byte >>3) & 0x1;
        header.join     = (*byte >>4) & 0x1;
        header.f_ack    = (*byte >>5) & 0x1;
        header.finger   = (*byte >>6) & 0x1;
...
void protokoll_debug(void *buffer, package_header header) {
    ...
    if (!header.intern) {
        ...
        printf("Intern:%d Finger:%d F_Ack:%d Join:%d Notify:%d stabilize:%d Reply:%d Lookup%d\n", header.intern, header.finger,
        header.f_ack,header.join, header.notify, header.stabilize, header.reply,header.lookup);
        printf("HASHID: %d ID: %d IP: %s Port: %d\n", header.hashID, header.id, ip4str, header.port);
        if(header.join){
            printf("\033[1;33m join request for ID %d PORT %d \033[0m\n",header.id,header.port);
        }else if(header.notify){
            printf("\033[1;33m Notify request for ID %d PORT %d \033[0m\n",header.id,header.port);
        }else if(header.stabilize){
            printf("\033[1;33m Stabilize request for ID %d PORT %d \033[0m\n",header.id,header.port);
        }
    }
    // else{
