...
int connect_to(const char *dns, const char *port) {
    ...
    // getaddrinfo call + fehlerbehandlung
    // printf("getaddrinfor for port %s\n",port);
...
int receive_data(int socket_fd, void **datapointer) {
    int bytes_recv = 0;                            // rueckgabewert von receive: bytes die empfangen wurden
    int bytes_total = 0;
    int package_size = 0;
    ...
    while ((bytes_recv = recv(socket_fd, buffer, BUFSIZE, 0)) != 0 && clock() < start_time + 2000) {
        if (bytes_recv == -1) {
            free(data);free(buffer);return -1;
        }
        ...
        if (!package_size) {
            ...
            package_size = (signed int) (7 + header.key_size + header.value_size + (header.intern * 4));
        }
        if(header.finger) break;
    ...
    *datapointer = data;
    ...
    // printf("receive data: received bytes_total %d \n",bytes_total);
    return bytes_total;
}
...
void translate_ip_and_port(const char* dns, const char* portstr, uint32_t *ip4, unsigned short *port) {
    ...
    // getaddrinfo call + fehlerbehandlung
    if ((rv = getaddrinfo(dns, portstr, &hints, &servinfo)) != 0) {
        printf("getaddrinfo error: line 190 %s\n", gai_strerror(rv));
        *ip4 = 0;
        *port = 0;
        return;
    }

    *ip4 = ((struct sockaddr_in *) servinfo->ai_addr)->sin_addr.s_addr;
