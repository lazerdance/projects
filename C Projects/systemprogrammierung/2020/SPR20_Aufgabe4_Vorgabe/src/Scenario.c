...
void * Scenario_Main(void * scenario_p)
{
	...
	Scenario * s = (Scenario *) scenario_p;
	printf("starting main for %d\n",s->args.iteration_num);
	// D) Finish up this function, use the synchronization data in args struct!
	
	// s->args.state = WORKING;
	// get scenario pointer

	while (true)
	{
		pthread_mutex_lock(s->args.mutex);
		while (*s->args.state == NEXT_STATE){
			pthread_cond_wait(s->args.signal_next, s->args.mutex);
		}
		// wait for next iteration
		// check for preemptive exit
		if (*s->args.state == EXIT){
			pthread_mutex_unlock(s->args.mutex);
			break;
		}

		// calculate next state
		if (NEXT_STATE == *s->args.state){
			// *s->args.state = NEXT_STATE;
			Scenario_NextState(s);
			pthread_cond_signal(s->args.signal_next);
		}

		pthread_mutex_unlock(s->args.mutex);

		// if(s->iteration_calc >= s->args.iteration_num){
		// 	break;
		// // signal worker done
		// pthread_cond_signal(&s->args.signal_done);
		// s->args.state = FINISHED;
